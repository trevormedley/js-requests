    ...
    axios
        ...
        .then((response) => {
            for (let i = 0; i < response.data.length; i++) {
                ...
                newPara.textContent = response.data[i];
...
function requestOne () {
    axios
        .get('http://localhost:3000/query-test')
        .then((response) => {
            console.log(response.data)
        })
}
...
queryButton.addEventListener("click", requestOne);
...
function createFood () {
    let foodInput = document.querySelector('#input1')
    let body = {
    ...
    axios
        ...
        .then((response) => {
            ...
            newFood.textContent = response.data;
...
addList.addEventListener('click', createFood)